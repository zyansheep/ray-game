...
impl Plugin for OrbitCameraPlugin {
    fn build(&self, app: &mut AppBuilder) {
        app.insert_resource(self.initial_state.clone()) // Initial state of camera
...
impl CameraFocusEvent {
    pub fn new(target: Vec3, interpolation: CameraInterpolation) -> Self {
        Self {
            target,
            interpolation,
        }
...
fn orbit_camera(
    ...
    if update_camera {
        ...
        camera_transform.translation =
            orbit_camera.focus + rot_matrix.mul_vec3(Vec3::new(0.0, 0.0, orbit_camera.distance));
    }
}
